function generateBubble(settings) {
    ...
    d3.csv(settings.path, function(error, data) {
        ...
        // ---------------------------//
           ...
           // Add X axis
            var x = d3.scaleTime()
                .domain(d3.extent(data, function(d){ return parseDate(d.publication_date); }))
                .range([0, width]);
            ...
            var y = d3.scaleLinear()
                .domain([2, 5]) // fixed rating - from 2 to 5
                .range([height, 0]);
                bubble_svg.append("g")
                .call(d3.axisLeft(y));
            ...
            var z = d3.scaleSqrt()
                .domain([0, d3.max(data.map(function(d) {return parseInt(d.num_pages);}))])
                .range([2, 30]);
        ...
        bubble_svg.append('g')
            ...
            .attr("cx", function (d) {  return x(parseDate(d.publication_date)); })
